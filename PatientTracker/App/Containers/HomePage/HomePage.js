    ...
    search = (text) => {
        if (text !== '') {
            ...
            let patientsbyName = this.props.patients.filter((patient) => {
                ...
                return patient.name.toLocaleLowerCase().indexOf(text.toLocaleLowerCase()) !== -1
            });
                    ...
                    <List>
                        {
                            !patients || patients.length === 0 ? loading ?
                                ...
                                patients.map((patient, i) => {
                                    ...
                                    return (
                                        <ListItem onPress={() => { navigate('PatientDetails', patient) }} key={i} style={{ marginLeft: 10, marginRight: 10 }}>
                                            <Thumbnail square size={80} source={require('../person.gif')} />
                                            <Body style={{ marginLeft: 10, }}>
                                                <Text>{patient.name}</Text>
                                                <Text note>{patient.date}</Text>
                                            </Body>
                                        </ListItem>
                                    )
                                })
